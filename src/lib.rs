...
fn for_all_shapes<F: FnMut(usize, usize) -> ()>(
...
fn count_shapes(kernel: &ArrayView2<bool>, data: &ArrayView2<bool>) -> usize {
  let mut result = 0;
  for_all_shapes(kernel, data, |_, _| result += 1);
  return result;
}

pub fn parse_input(filename: &str) -> Vec<Tile> {
  return parse_tiles(
...
pub fn assemble_tiles(tiles: &Vec<Tile>) -> Array2<bool> {
...
pub fn count_monsters(image: &Array2<bool>, verbose: bool) -> usize {
...
#    ##    ##    ###
 #  #  #  #  #  #   "
  ...
  for i in 0..8 {
    let monster_count = count_shapes(&monster.view(), &result_tile.data);
    if monster_count > 0 {
      if verbose {
        let mut monsters = Array2::from_elem(result_tile.data.raw_dim(), false);
        for_all_shapes(&monster.view(), &result_tile.data, |i, j| {
          monsters
            .slice_mut(s![i..i + monster.shape()[0], j..j + monster.shape()[1]])
            .assign(&monster.view())
        });

...
fn count_neighbors(state: &Array3<bool>) -> Array3<u8> {
  let mut result: Array3<u8> = Array3::zeros(state.raw_dim());
  let shape = result.raw_dim();

  let bounded_range =
    |d, i| (if i == 0 { 0 } else { -1 as isize })..(if i + 1 == shape[d] { 1 } else { 2 });

  for ((i, j, k), value) in result.indexed_iter_mut() {
...
fn update_state(state: &mut Array3<bool>) {
...
fn main() {
  ...
  let mut state: Array3<bool> = Array3::from_elem(final_size, false);
  for i in 0..input.len() {
    for j in 0..input[0].len() {
      state[[
        final_size.0 / 2,
        final_size.1 / 2 - input.len() / 2 + i,
        final_size.2 / 2 - input[0].len() / 2 + j,
      ]] = input[i][j];
    }
  }

  let print_state = |i, state: &Array3<bool>| {
    println!(
      "cycle {}, {} active cubes",
      i,
